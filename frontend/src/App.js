...
function App() {
  ...
  const search = async (patch) => {
    try {
      const patchTargetResponse = await fetch("http://localhost:4567/search", { method: "POST", body: patch })
      ...
      const byContentResponse = await fetch("http://localhost:4567/search?by_content=true", { method: "POST", body: patch })
